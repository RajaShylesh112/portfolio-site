...
import type { BlogEntryRecord } from "@/lib/blog-store";

export default function Blog() {
  const { blogEntries } = useBlogs();
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedCategory, setSelectedCategory] = useState('all');
  const [selectedEntry, setSelectedEntry] = useState<BlogEntryRecord | null>(null);
  ...
  const handleEntryClick = (entry: BlogEntryRecord) => {
